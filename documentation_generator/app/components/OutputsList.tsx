...
const OutputsList: FC<Props> = ({ outputs, selectedOutputId, onSelect, onDownload }) => {
  ...
  return (
    <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: 8 }}>
      {outputs.map((out) => (
        <li key={out.id} onClick={() => onSelect(out.id)} style={{ border: out.id === selectedOutputId ? "1px solid var(--primary)" : "1px solid var(--border)", background: out.id === selectedOutputId ? "var(--panel-bg-selected)" : "var(--panel-bg)", borderRadius: 10, padding: 10, cursor: "pointer", display: "flex", flexDirection: "column", gap: 8 }}>
